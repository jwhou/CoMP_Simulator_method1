...
global RTS_method CoMP_mode;
global lastsounding_enable;
global scheme;

% scheme_quantity = [1,2,3];
scheme_quantity = [3]; % testing by shuyu 
RTS_quantity = [0,1]; %testing by jing-wen  0: no RTS, 1: Typical RTS, 2: RTS Simultaneously
CoMP_quantity = [1]; %testing by jing-wen  0: No CoMP, 1: CS/CB, 2: JT, 3: Network MIMO
...
for ind0=1:length(run_quantity)
    ...
    for ind1=1:length(scheme_quantity)
        ...
        for ind2=1:length(RTS_quantity)
            RTS_method = RTS_quantity(ind2);
            for ind3=1:length(CoMP_quantity)
                if (scheme == 1 && ind3 > 1), continue; end
                CoMP_mode = CoMP_quantity(ind3);
                % ======================== Block 2 ========================
                ...
                elseif (scheme == 2)
                    if (CoMP_mode == 1)
                        disp(['======== Scheme: CS/CB method 1 @ '  num2str(datestr(now))]);
                        fid = fopen(['outputs_traffic', num2str(traffic_type),'_CSCB_M1.txt'],'a');
                        CoMP_Controller = CoMP_Pair_Algo(Num_AP, Num_User, XY_vector, 1, envAPmodel, tho, Area);
                    elseif (CoMP_mode == 2)
                        disp(['======== Scheme: JT method 1 @ '  num2str(datestr(now))]);
                        fid = fopen(['outputs_traffic', num2str(traffic_type),'_JT_M1.txt'],'a');
                        CoMP_Controller = CoMP_Pair_Algo(Num_AP, Num_User, XY_vector, 1, envAPmodel, tho, Area);
                    elseif (CoMP_mode == 3)
                    end
                elseif (scheme == 3)
                    if (CoMP_mode == 1)
                        disp(['======== Scheme: CS/CB method 2 @ '  num2str(datestr(now))]);
                        fid = fopen(['outputs_traffic', num2str(traffic_type),'_CSCB_M2.txt'],'a');
                        CoMP_Controller = CoMP_Pair_Algo(Num_AP, Num_User, XY_vector, 1, envAPmodel, tho, Area);
                    elseif (CoMP_mode == 2)
                        disp(['======== Scheme: JT method 2 @ '  num2str(datestr(now))]);
                        fid = fopen(['outputs_traffic', num2str(traffic_type),'_JT_M1.txt'],'a');
                        CoMP_Controller = CoMP_Pair_Algo(Num_AP, Num_User, XY_vector, 1, envAPmodel, tho, Area);
                    elseif (CoMP_mode == 3)
                    end
                ...
                while 1
                    ...
                    NewEvents = action_CoMP_method1(Event_list(min_index),PHY_CH_module,Nsubcarrier,channel_model,Bandwidth,Thermal_noise,tx_ant_gain,reflection_times,wall_mix,wall_index,HOV,room_range,AI);
                ...
                if RTS_method == 0
                    fprintf(fid, 'RTS_method = No RTS/CTS\r\n');
                elseif RTS_method == 1
                    fprintf(fid, 'RTS_method = Typical RTS/CTS\r\n');
                elseif RTS_method == 2
                    fprintf(fid, 'RTS_method = RTS Simultaneously\r\n');
                end
...
% set(gca,'XTickLabel', {'DL MU-MIMO', 'CS/CB_M1' ,'CS/CB_M2'});
% hold off
% ====================== Block 4 end ======================
