...
for i=1:numSTAs
    ...
    if (i <= Num_AP)
        ...
        STA_Info(i).Precoder_record = [];           %added by jing-wen
        STA_Info(i).Channel_record = [];            %added by jing-wen
        STA_Info(i).SNR_record = zeros(1, Num_User);%added by jing-wen
        STA_Info(i).soundingqueue = [];             %added by jing-wen
